...
class AIClient:
    ...
    def generate_text(self, prompt, provider=None, system_prompt=None):
        ...
        if target_provider == "gemini" and self.gemini_key:
            ...
            return f"[AI Client Note: API Key not set for {target_provider}. Please configure GEMINI_API_KEY, OPENAI_API_KEY, or ANTHROPIC_API_KEY in .env file.]"
    ...
    def parse_natural_language_query(self, user_query):
        """Parses a user search prompt into structured search criteria using AI or smart regex fallback."""
        ...
        # Fallback if no API key is active yet
